...
import { createAddress, stringUtf8CV } from '@stacks/transactions'
...
//      if (/^S[PT]([0-9A-Z]{38,40})/.test(searchid)) {
//        console.log('validated stacks address format: ' + searchid)
//      }
        try {
          const parsedAddress = createAddress(searchid);
        } catch {
          try {
            // not a stacks address, maybe a BNS domain name
            const ownerAddress = await getOwner({
              fullyQualifiedName: searchid,
              network: network,
            });
